...
export class Cao__proposalVotesResult {
  value0: BigInt;
  value1: BigInt;
  value2: BigInt;
...
export class Cao extends ethereum.SmartContract {
  static bind(address: Address): Cao {
    return new Cao("Cao", address);
  }
  ...
  proposalVotes(proposalId: BigInt): Cao__proposalVotesResult {
    let result = super.call(
      "proposalVotes",
    ...
    return new Cao__proposalVotesResult(
      result[0].toBigInt(),
      result[1].toBigInt(),
      result[2].toBigInt()
    );
  ...
  try_proposalVotes(
    proposalId: BigInt
  ): ethereum.CallResult<Cao__proposalVotesResult> {
    let result = super.tryCall(
      "proposalVotes",
      "proposalVotes(uint256):(uint256,uint256,uint256)",
      [ethereum.Value.fromUnsignedBigInt(proposalId)]
